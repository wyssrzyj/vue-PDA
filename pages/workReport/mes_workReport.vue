	...
	<view class="mainContent" @click="handleClcik">
				...
				<view class="storageInfo">
					...
					<view class="storageColor">
						...
						<text style="margin-left: 80rpx">
							<text style="font-weight: bold">床号：</text>{{ item.bedNum }}
						</text>
						<text style="margin-left: 80rpx">
							<text style="font-weight: bold">包号：</text>{{ item.packageNum }}
						</text>
					</view>
		...
		<view class="bottomTab">
			...
			<view class="bottom-right">
				...
				<view
					v-if="outStorageArr.length > 0"
					id="outButton"
					class="btn btnActive"
					:loading="loading"
					:disabled="loading"
					@click="handleOutStorage"
				>
		...
		<!-- 选择工序弹窗 -->
		<select-code-multiple
			:visible="showM"
			:checkedValue="checkedList"
			:optionList="coutryList"
			@confirm="getCodeMu"
			@cancel="showM = false"
		/>
		<!-- 选择工段弹窗 -->
		<u-action-sheet
			:actions="sectionList"
			:show="showSection"
			@select="sectionSelectClick"
			:closeOnClickOverlay="true"
			:closeOnClickAction="true"
			@close="showSection = false"
		/>
		<scan-code />
		<!-- 选择用户 -->
		<searchSelect
		...
		<workReportDialog
			:show="showDialog"
			:infoList="dialogInfoList"
			@close="showDialog = false"
			@delPackage="delPackage"
			@clearPackage="selectClick({ name: '清空' })"
		/>
	</view>
</template>
...
let message = (msg, time = 3000) =>
	uni.showToast({ icon: "none", duration: time, title: msg });
...
export default {
	...
	methods: {
		...
		async selectSure(val) {
			...
			this.userId = val.id;
			if (this.type == "1") this.getCache(this.userId);
			const data = {
				userId: val.id,
				id: this.produceId,
			};
		...
		getCache(userId, autoReporting = false) {
			Api.getStaffInfoCache({ userId, produceId: this.produceId || 1 }).then(
				(res) => {
					if (res.code !== 0) {
					...
					if (oldCheckedList.length) {
						...
						if (sectionList.includes(oldSection)) {
							this.sectionSelectClick({ name: oldSection }); //设置工序列表
