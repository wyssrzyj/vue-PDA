	...
	<view class="mainContent">
		...
		<view class="borderBottom">
			...
			<view class="storageLocation">
				...
				<input class="uni-input storageInput" placeholder-style="font-size: 34rpx" v-model="sewingTaskRecord" confirm-type="search" placeholder="请输入产品款号" disabled/>
				<view class="queryBtn" @tap="showProductNumModel===true">查询</view>
			</view>
			<view class="storageLocation">
				<text style="color: red;">*</text><text class="storageTitle">供应商简称：</text>
				<input class="uni-input storageInput" placeholder-style="font-size: 34rpx" v-model="sewingTaskRecord" confirm-type="search" placeholder="请输入供应商简称" disabled/>
		...
		<view class="bottomLocation">
			<view class="scanNum"><view><text class="scannedNum">{{ alreadyOutStorageArr.length }}</text>/{{ outStorageArr.length }}</view><view><text>已扫描数量：</text><text class="scannedAllNum">0</text></view></view>
			<view class="btnLocation">
				<view class="commonBtn moreBtn" @tap="handleMore">更多</view>
				<view class="commonBtn inStorageBtn" @tap="handleOutStorage" v-if="outStorageArr.length > 0 && outStorageArr.length == alreadyOutStorageArr.length">出库并打印</view>
		...
		<view class="productNumModel" v-if="showProductNumModel">
			<input placeholder-style="font-size: 34rpx" v-model="sewingTaskRecord" confirm-type="search" placeholder="产品款号"/>
			<input placeholder-style="font-size: 34rpx" v-model="sewingTaskRecord" confirm-type="search" placeholder="客户款号"/>
